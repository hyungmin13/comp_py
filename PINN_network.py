...
class Seperable(Network):
    ...
    def init_params(key, layer_sizes,  network_name):
        keys = random.split(key, len(layer_sizes)-1)
        params = [[eval(network_name)._random_layer_params(k, m, n)
                  for k, m, n in zip(keys, layer_sizes[:-1], layer_sizes[1:])] for _ in range(4)]
        network_params = {"layers": params, "layer_sizes":layer_sizes, "network_name":network_name}
        return network_params
    ...
    @staticmethod
    def separate_net(inmin, inmax, x, param):
        H = 2*(x.copy() - inmin)/(inmax-inmin) - 1
        for w, b, g in param[:-1]:
            H = g*jnp.dot(H,w/jnp.linalg.norm(w,axis=0, keepdims=True)) + b
            H = jnp.tanh(H)
        w, b, g = param[-1]
        H = jnp.dot(H,w/jnp.linalg.norm(w,axis=0, keepdims=True)) + b        
        return H
    ...
    def network_fn(all_params, x):
        ...
        inmin = all_params["domain"]["in_min"]
        inmax = all_params["domain"]["in_max"]
        upred = Seperable.separate_net(inmin, inmax, x, params[0])
        vpred = Seperable.separate_net(inmin, inmax, x, params[1])
        wpred = Seperable.separate_net(inmin, inmax, x, params[2])
        ppred = Seperable.separate_net(inmin, inmax, x, params[3])
        preds = jnp.concatenate([upred, vpred, wpred, ppred], axis=1)
        return preds
